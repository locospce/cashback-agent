...
SYSTEM_PROMPT = (
    "Ты старший стратег кэшбэк-программы банка. "
...
def _build_prompt(
    ...
    for i, (r, p) in enumerate(zip(analyst_results, params), 1):
        ...
        lines += [
            ...
            f"кэшбэк {m['total_cashback']:,.0f} руб ({m['cashback_load_pct']:.1f}%), "
    ...
    lines += [
        ...
        "2. Рекомендуемый % кэшбэка для каждого сегмента\n"
    ...
    _analyst_results = [
        {
            ...
            "summary": "Кампания прошла стабильно, нагрузка кэшбэка в норме.",
        },
    ]
    _forecast = {
